...
{
private:
    using self_type = SharedMemoryMap<Key, T, Compare, Allocator>;
    ...
    private:
        friend class SharedMemoryMap<Key, T, Compare, Allocator>;
...
public:
    ...
    explicit SharedMemoryMap(const allocator_type& alloc) noexcept
        : allocator(alloc), node_allocator(alloc.get_manager()), compare(), root(nullptr), size_(0)
    {
    }

    SharedMemoryMap(const key_compare& comp, const allocator_type& alloc) noexcept
        : allocator(alloc), node_allocator(alloc.get_manager()), compare(comp), root(nullptr),
          size_(0)
    {
    }

    SharedMemoryMap(const SharedMemoryMap& other)
    ...
    SharedMemoryMap(SharedMemoryMap&& other) noexcept
        : allocator(other.allocator), node_allocator(other.allocator.get_manager()),
    ...
    mapped_type& at(const key_type& key)
    {
        ...
        if (it == end())
        {
            throw std::out_of_range("SharedMemoryMap::at key not found");
        }
        return it->second;
    }
...
private:
    ...
    void copy_from(const SharedMemoryMap& other)
    {
